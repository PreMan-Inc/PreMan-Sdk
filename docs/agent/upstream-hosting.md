...
| `updateHostedMcp()` | `PATCH /hosted-mcps/{id}` (`name`, `llms_txt_markdown`, …) |

Hosted MCP `llms_txt_markdown` is stored on the MCP row and served publicly at `GET /h/{id}/llms.txt`. The dashboard overview renders it below the activity graph.
